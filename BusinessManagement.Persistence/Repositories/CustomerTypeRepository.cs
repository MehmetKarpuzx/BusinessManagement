using BusinessManagement.Application.Interfaces.Repositories;
using BusinessManagement.Domain.Entities;
using BusinessManagement.DTO.CustomerTypeDTOs;
using BusinessManagement.Persistence.Contexts;
using Microsoft.EntityFrameworkCore;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace BusinessManagement.Persistence.Repositories
{
    public class CustomerTypeRepository : ICustomerTypeRepository
    {
        private readonly BusinessManagementDbContext _context;
        public CustomerTypeRepository(BusinessManagementDbContext context)
        {
            _context = context;
        }

        public async Task<AddCustomerTypeDto> AddCustomerTypeAsync(AddCustomerTypeDto dto)
        {
            var entity = new CustomerType
            {
                Name = dto.Name,
                Description = dto.Description,
                CreateDate = dto.CreateDate
              
            };
            _context.CustomerTypes.Add(entity);
            await _context.SaveChangesAsync();
            return dto;
        }

        public Task DeleteCustomerTypeAsync(int id)
        {
            var entity = _context.CustomerTypes.Find(id);
            if (entity == null || entity.IsDeleted)
            {
                throw new KeyNotFoundException($"CustomerType bulunamadı ID = {id}  (404)");
            }
            entity.IsDeleted = true;
            _context.CustomerTypes.Update(entity);
            return _context.SaveChangesAsync();
        }

        public async Task<List<ResponseCustomerTypeDto>> GetAllAsync()
        {
            var entities = await _context.CustomerTypes
                .Where(b => !b.IsDeleted)
                .Select(b => new ResponseCustomerTypeDto
                {
                    Id = b.Id,
                    Name = b.Name,
                    Description = b.Description,
                    CreateDate = b.CreateDate,
                    IsDeleted = b.IsDeleted
                })
                .ToListAsync();
            return entities;    
        }

        public async Task<UpdateCustomerTypeDto> UpdateCustomerTypeAsync(int id, UpdateCustomerTypeDto dto)
        {
            var entity = await _context.CustomerTypes.FindAsync(id);
            if (entity == null || entity.IsDeleted)
            {
                throw new KeyNotFoundException($"CustomerType bulunamadı ID = {id}  (404)");
            }
            var updatedEntity = new CustomerType
            {
                Id = id,
                Name = dto.Name,
                Description = dto.Description,
                IsDeleted = dto.IsDeleted
            };
            _context.Entry(entity).CurrentValues.SetValues(updatedEntity);
            await _context.SaveChangesAsync();
            return dto;
        }
    }
}
